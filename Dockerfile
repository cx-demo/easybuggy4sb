...
RUN if [ "$env" = "TEST" ] ; then \
    curl -o cxiast-java-agent.zip http://${ENV_IAST_URL}/iast/compilation/download/JAVA && \
...
ENV JAVA_TOOL_OPTIONS=${env:+"-javaagent:/home/easybuggy4sb/cxiast-java-agent/cx-launcher.jar -Xverify:none"} 
